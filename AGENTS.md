...
Cargo workspace (`resolver = "2"`): `server` (Axum + maud) and `client-wasm` (wasm-bindgen). HTMX is vendored at `static/htmx.min.js` — no SPA framework.
...
- Shared IPv4 + dedicated IPv6 already allocated. **Do not** `fly ips allocate-v4` without `--shared` ($2/mo dedicated). This app has no volumes and no database — do not add either unless the task needs them.
...
- Pages live in `server/src/pages.rs` and `server/src/layout.rs`. Match existing maud + HTMX patterns; do not add a JS framework.
