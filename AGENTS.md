...
- Shared IPv4 + dedicated IPv6 already allocated. **Do not** `fly ips allocate-v4` without `--shared` ($2/mo dedicated). No volumes. Postgres is the existing MPG cluster **postgreputest** (fra): database `fossall` / schema `words`, and database `earth` / schema `earth`. **Do not** create a new cluster. Attach earth with `--variable-name EARTH_DATABASE_URL` so words `DATABASE_URL` stays intact (`make earth-mpg-setup`). MPG setup needs an org-scoped Fly token (`FLY_ORG_TOKEN` / `fly tokens create org`); a deploy-only `FLY_API_TOKEN` cannot see the cluster. See [docs/DATABASE.md](docs/DATABASE.md) and [docs/EARTH.md](docs/EARTH.md). Never paste `fly mpg status --json`.
