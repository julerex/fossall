...
- Shared IPv4 + dedicated IPv6 already allocated. **Do not** `fly ips allocate-v4` without `--shared` ($2/mo dedicated). No volumes. Postgres is the existing MPG cluster **postgreputest** (fra): database `fossall` / schema `words`, and database `earth` / schema `earth`. **Do not** create a new cluster. Attach earth with `--variable-name EARTH_DATABASE_URL` so words `DATABASE_URL` stays intact (`make earth-mpg-setup`). Creating database `earth` on this MPG v1 cluster is a one-time Fly dashboard action if `fly mpg databases create` 404s; after that the GitHub Action can apply schema, attach, and seed. See [docs/DATABASE.md](docs/DATABASE.md) and [docs/EARTH.md](docs/EARTH.md). Never paste `fly mpg status --json`.
